package com.sahaj.banking.client;

import com.sahaj.banking.entity.AccountManager;
...
public class BankUserInterface {
    public static void main(String[] args) throws FileNotFoundException {
        ...
        String inputCommand;
        String[] inp;
        AccountManager sahajAccMgr = new AccountManager();
        ...
        while(!exit) {
            try {
                ...
                if (cmd.equals("create")) {
                    ...
                    sahajAccMgr.createAccount(name);
                }
                    ...
                    sahajAccMgr.depositInAccount(accID, amount);
                }
                else if (cmd.equals("withdraw")) {
                    long accID = Long.parseLong(inp[1]);
                    double amount = Double.parseDouble(inp[2].indexOf('.')==-1?inp[2]:inp[2].substring(0, min(inp[2].indexOf('.')+3, inp[2].length())));
                    sahajAccMgr.withdrawFromAccount(accID, amount);
                }
                else if (cmd.equals("balance")) {
                    long accID = Long.parseLong(inp[1]);
                    sahajAccMgr.getBalanceOfUser(accID);
                }
                    ...
                    sahajAccMgr.transferFunds(srcAccID,destAccID, amount);
                }
